...
StepResult Controller::calculate_step(StepParams params) {
  ...
  switch (state) {
    case INITIAL_STATE:
      ...
      fi << "Impulse 3: " << third_impulse_speed << std::endl;
      fi << "Radius 1:" << new_radius1 << std::endl;
      fi << "Radius 2:" << new_radius2 << std::endl;
      fi.close();
#endif
...
/*
      if (task == TASK_ZERO or task == TASK_TWO)
      ...
      if (is_between(previous_pos, Vec2(cos(required_angle / 180 * M_PI), sin(required_angle / 180 * M_PI)), params.position)) {
        ...
        if (task == TASK_ONE)
          complete = true;
            ...
            fi << "Time: " << params.time << " State: " << state << " Set state " << WAIT_FOR_THIRD_IMPULSE << std::endl;
            fi.close();
#endif
